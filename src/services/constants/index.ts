...
export const IS_SAMSUNG =
  IS_ANDROID &&
  /SAMSUNG|SGH-[I|N|T]|GT-[I|P|N]|SM-[N|P|T|Z|G]|SHV-E|SCH-[I|J|R|S]|SPH-L/i.test(
    USER_AGENT
  );

export const IS_CYPRESS = IS_CLIENT_SIDE
  ? Object.hasOwn(window, "Cypress")
  : false;

export const IS_MAC =
  !!navigator.platform && navigator.platform.toLowerCase().indexOf("mac") >= 0;
export const IS_WINDOWS =
  !!navigator.platform && navigator.platform.indexOf("Win") > OUT_OF_RANGE;
export const IS_LINUX =
  !!navigator.platform && navigator.platform.indexOf("Linux") > OUT_OF_RANGE;
export const IS_WINDOWS_10 =
  IS_WINDOWS && USER_AGENT.indexOf("Windows NT 10.0") > OUT_OF_RANGE;
...
function getBrowser() {
  ...
  if (
    USER_AGENT.indexOf("FBAN") > OUT_OF_RANGE ||
    USER_AGENT.indexOf("FBAV") > OUT_OF_RANGE
  ) {
    return "facebook";
  }
  if (
    USER_AGENT.indexOf("Opera") > OUT_OF_RANGE ||
    USER_AGENT.indexOf("OPR") > OUT_OF_RANGE
  ) {
    return "opera";
  }
  if (
  ...
  if (
    USER_AGENT.indexOf("Firefox") > OUT_OF_RANGE &&
    USER_AGENT.indexOf("Seamonkey") === OUT_OF_RANGE
  ) {
    return "firefox";
  }
  return null;
